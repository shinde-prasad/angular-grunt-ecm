...
searchApp.directive('participantNotesHistoryTable', function($compile, $filter) {
    return {
        restrict: 'AE',
        replace: 'true',
        template: '<div></div>',
            ...
            return function (scope, element) {
                ...
                if((scope.notesData) && (scope.notesData.length > 0)) {
                    ...
                    for (var i = 0; i < Object.getOwnPropertyNames(scope.notesData[0]).length; i++) {
                        ...
                        if(scope.notesDiscardedFields.indexOf(currHeader) === -1) {    //If the header is not in the list of discarded fields, then add the data
                    ...
                    for (var i = 0; i < scope.notesData.length; i++) {
                        ...
                        for (var j = 0; j < headers.length; j++) {
                            if(scope.notesDiscardedFields.indexOf(headers[j]) === -1) {    //If the header is not in the list of discarded fields, then add the data
